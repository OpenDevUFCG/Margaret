...
class SubscribedsController:
    ...
    def find_subscribed_by_attribute(self, attribute, value_attribute):
        if attribute.lower() not in ['name', 'email', 'discord_id', 'period',
                                     'minoritary_group']:
    ...
    def modify_subscribed_by_attribute(self, email_user, attribute,
                                       new_attribute):
        if attribute.lower() not in ['name', 'email', 'discord_id', 'period',
                                     'minoritary_group']:
            raise AttributeError("Atributo inválido")
    ...
    def get_subscribed(self, email_user):
        ...
        if email_user not in self.subscribers.keys():
