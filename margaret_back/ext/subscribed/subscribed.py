...
def modify_sub(email_sub):
    ...
    data = request.get_json()
    try:
        subsController.modify_subscribed_by_attribute(
            email_sub, data['attribute'], data['new_attribute'])
    except AttributeError:
        abort(400, "Atributos inválidos")
    except ValueError:
        abort(404, "Usuário não inscrito")
...
def search_sub():
    data = request.get_json()
    try:
        founds = subsController.find_subscribed_by_attribute(
            data['attribute'], data['value_attribute'])
    except AttributeError:
        abort(400, "Atributos inválidos")
