...
def test_subscribed_create_sucess():
    sub = Subscribed('Midorya', 'deku.arumairo@ccc.ufcg.edu.br',
                     'Deku#1969', '2016.2', 'LGBTQA+')
...
def test_sbuscribed_empty_minority_group():
    sub = Subscribed('Todoroki', 'todoroki.shoto@ccc.ufcg.edu.br',
                     'Shoto#8445', '2015.1', '')
...
def test_subscribed_empty_period():
    try:
        sub = Subscribed(
            'Uraraka', 'uraraka.gravity@ccc.ufcg.edu.br', 'Uravity#4576', '')
...
def test_subscribed_letters_period():
    try:
        sub = Subscribed('Lida', 'lida.runner@ccc.ufcg.edu.br',
                         'Runner#4576', 'doismilequize.1')
        fail('Periodo vazio foi cadastrado')
    except AttributeError:
        pass
...
def test_subscribed_period_without_semester():
    try:
        sub = Subscribed(
            'Bakugo', 'bakugo.numberone@ccc.ufcg.edu.br', 'Boom#4576', '2015')
